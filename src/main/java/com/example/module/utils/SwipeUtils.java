...
public class SwipeUtils {
     ...
     * 获取View中心点坐标
     */
    private float[] getPoint(View view) {
        int[] locs = new int[2];
     ...
     * 获取View中心点坐标
     */
    public float[] getCenPoint(View view) {
        int[] locs = new int[2];
        view.getLocationOnScreen(locs);
        //中心点
     ...
     *                 {@link #HIGH} 高
     */
    public static void simulateScroll(View activity, float startX, float startY, float endX, float endY, long duration, int period) {
        dealSimulateScroll(activity, startX, startY, endX, endY, duration, period);
    }

    private static void dealSimulateScroll(View object, float startX, float startY, float endX, float endY, long duration, int period) {
        long downTime = SystemClock.uptimeMillis();
        Handler handler = new ViewHandler(object);

        object.dispatchTouchEvent(createFingerMotionEvent(downTime, downTime, MotionEvent.ACTION_DOWN, startX, startY, 0));
    ...
    static class ViewHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (count >= bean.totalCount) {
                theView.dispatchTouchEvent(createFingerMotionEvent(downTime, downTime, MotionEvent.ACTION_UP, bean.endX, bean.endY, 0));
            } else {
                theView.dispatchTouchEvent(createFingerMotionEvent(downTime, downTime, MotionEvent.ACTION_MOVE, bean.startX + bean.ratioX * count, bean.startY + bean.ratioY * count, 0));
                bean.count++;
    ...
    static class GestureBean {
        ...
        GestureBean(float startX, float startY, float endX, float endY, long duration, int speed) {
            ...
            this.endY = endY;
            this.period = speed;
            totalCount = (duration / speed)+20;
            ratioX = (endX - startX) / totalCount;
            ratioY = (endY - startY) / totalCount;

            //CLogUtils.e("总共需要多少次处理事件 "+totalCount);
        }
    }
     ...
     */
    public static MotionEvent createFingerMotionEvent(long downTime, long eventTime, int action, float x, float y, int metaState) {
        ...
        MotionEvent motionEvent = MotionEvent.obtain(downTime, eventTime, action, 1, pointerPropertiesArray, pointerCoordsArray, 0, 0, 0, 0, 7, 0, InputDevice.SOURCE_TOUCHSCREEN, 0);
        return motionEvent;
    }
}
