...
/**
 * @author Zhenxi on 2020-06-17
 */
public class ForceFiledViewer {
    /**
     * build a plain object from any flex object,the output is a view ,
     * witch access all private and public field,ignore getter on input object
     *
    ...
    private static Object toViewInternal(Object input, Set<Object> accessedObjects, boolean withObjType) {
        ...
        // primary type,
        // 判断是否有包装类
        if (inputClass.isPrimitive() || isWrapperType(inputClass) ) {
            return input;
        }
        ...
        if (input instanceof Collection) {
            List<Object> subRet = new ArrayList<>(((Collection) input).size());
            boolean hint = false;
        ...
        if (input.getClass().isArray()) {
            List<Object> subRet = new ArrayList<>(Array.getLength(input));
            boolean hint = false;
        ...
        if (input instanceof Map) {
            ...
            for (Object entry : set) {
                ...
                if (view != null) {
                    if (key instanceof CharSequence || key.getClass().isPrimitive() || isWrapperType(key.getClass())) {
    ...
    public static class PackageTrie {
        ...
        public void addToTree(String packageName) {
            addToTree(new ArrayList<>(split(packageName)), 0);
        }

        public boolean isSubPackage(String packageName) {
            return isSubPackage(new ArrayList<>(split(packageName)), 0);
        }
    ...
    private static Method getDeclaredFieldsMethod;
    ...
    public static Map<String, Field> classFileds(Class clazz) {
        ...
        synchronized (clazz) {
            ...
            for (Field next : getDeclaredField(clazz)) {
                ...
                try {
                    if (gsonSerializeNameAnnotation != null && next.isAnnotationPresent(gsonSerializeNameAnnotation)) {
                        ...
                        String gsonName = (String) XposedHelpers.callMethod(annotation, "value");
                        if (gsonName != null) {
