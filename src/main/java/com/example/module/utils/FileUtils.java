...
public class FileUtils {
    ...
    public synchronized static void SaveNewFileString(Context mContext,String str, String FileName,String SdPath) {
        FileWriter fw = null;
        try {
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                File filedir = new File(sdCardDir + File.separator + SdPath+"/");  // 这里的AA为创建的AA文件夹，在根目录下
                if (!filedir.exists()) {
                    filedir.mkdirs();
                }
                ...
                try {
                    ...
                    CLogUtils.e("SaveNewFileString  保存 文件出错 "+e.getMessage()+"\n"+str);
                }
            }
        } catch (IOException e) {
            CLogUtils.e("SaveNewFileString  保存 文件出错 "+e.getMessage()+"\n"+str);
    ...
    public synchronized static void SaveString(Context mContext,String str, String packageName) {
        ...
        try {
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                // 2.1的时候为：/sdcard，所以使用静态方法得到路径会好一点。
                //String s=Environment.getExternalStorageDirectory();
                File filedir = new File(sdCardDir + File.separator + "Test/");  // 这里的AA为创建的AA文件夹，在根目录下
                if (!filedir.exists()) {
                    filedir.mkdirs();
                }
